...
/* Global variables to be used by toMultiThread and main */
int nthreads;
scene_t scene;
float print_scaled_color[width][height][3];

void *toMultiThread(void *threadID) {
    Vec3 camera_pos;
    ...
    /* for every pixel */
    for( int px=*(int *)threadID; px<width; px+=nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            /* store this pixel in array to print. ppm is forgiving about whitespace,
             * but has a maximum of 70 chars/line, so use one line per pixel
             */
            print_scaled_color[px][py][0] = scaled_color[0];
            print_scaled_color[px][py][1] = scaled_color[1];
            print_scaled_color[px][py][2] = scaled_color[2];
        }
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    for ( t=0; t<nthreads; t++ ) {
        thread_args[t] = t;
        ret = pthread_create( &threads[t], NULL, toMultiThread, &thread_args[t] );
        if ( ret ) {
    ...
    for ( i=0; i<width; i++) {
        for ( j=0; j<height; j++ ) {
            printf( "%.0f %.0f %.0f\n",
                   print_scaled_color[i][j][0],
                   print_scaled_color[i][j][1],
                   print_scaled_color[i][j][2] );
        }
        printf( "\n" );
    }
